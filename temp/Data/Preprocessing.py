...
def prepros_adult(train_file, val_file, method):
    # measure energy consumption
    # tracker = EmissionsTracker(project_name=method, measure_power_secs=1, log_level='warning', output_dir='emissions_log')

    # tracker.start_task("Inference")

    # load data based on input
    adult_train = pd.read_csv(train_file)
    adult_val = pd.read_csv(val_file)
...
def prepros_student(input_file, method):
    ...
    # load data
    student = pd.read_csv(input_file)
